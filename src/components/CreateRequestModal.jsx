...
export default function CreateRequestModal({
  ...
  // fetch AVAILABLE assets whenever the to_store changes or tab opens
  useEffect(() => {
    if (!form.to_store_id) return;
    setAssetsLoading(true);
    API.get(`/assets?status=AVAILABLE&store_id=${form.to_store_id}`)
      .then((r) => setAvailAssets(r.data.data || []))
      .catch(() => setAvailAssets([]))
      .finally(() => setAssetsLoading(false));
  }, [form.to_store_id]);

  // toggle an asset in/out of form.requested_assets (array of asset_id)
  const toggleAsset = (asset) => {
    setForm((f) => {
      const current = f.requested_assets || [];
      const exists = current.find((a) => a.asset_id === asset.asset_id);
      return {
        ...f,
        requested_assets: exists
          ? current.filter((a) => a.asset_id !== asset.asset_id)
          : [
              ...current,
              {
  ...
  const isSelected = (asset_id) =>
    (form.requested_assets || []).some((a) => a.asset_id === asset_id);
        ...
        <form onSubmit={onSubmit} className="p-5 space-y-4">
          ...
          <div className="flex rounded-lg border border-gray-200 overflow-hidden">
            ...
            >
              ...
              {form.items?.length > 0 && (
                <span
                  className={`ml-2 text-xs px-1.5 py-0.5 rounded-full ${activeTab === "items" ? "bg-white/20 text-white" : "bg-emerald-100 text-emerald-700"}`}
                >
                  {form.items.length}
                </span>
              )}
          ...
          {activeTab === "assets" && (
            <div>
              {!form.to_store_id ? (
                <div className="text-gray-400 text-xs text-center py-6 border border-dashed border-gray-300 rounded-lg">
                  Select a Main Store first to load available assets
                </div>
              ) : (
                <div className="space-y-3">
                  {/* Search */}
                  <input
                    ...
                    className="w-full bg-white border border-gray-300 rounded px-3 py-2 text-gray-800 text-sm focus:outline-none focus:border-blue-400"
                  />

                  {/* Selected summary */}
                  {(form.requested_assets || []).length > 0 && (
                    <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2">
                      <p className="text-blue-700 text-xs font-semibold mb-1.5">
                        ✓ {form.requested_assets.length} asset
                        {form.requested_assets.length > 1 ? "s" : ""} selected
                      </p>
                      <div className="flex flex-wrap gap-1.5">
                        {form.requested_assets.map((a) => (
                          <span
                            key={a.asset_id}
                            className="inline-flex items-center gap-1 bg-white border border-blue-200 text-blue-700 text-xs rounded px-2 py-0.5"
                          >
                            {a.asset_name}
                            <button
                              type="button"
                  ...
                  {assetsLoading ? (
                    ...
                    <div className="text-center py-8 text-gray-400 text-sm border border-dashed border-gray-200 rounded-lg">
                      {assetSearch
                        ...
                        : "No available assets in this store"}
                    </div>
                  ) : (
                    <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                      {filteredAssets.map((a) => {
                        const selected = isSelected(a.asset_id);
                        return (
                          <div
                            key={a.asset_id}
                            onClick={() => toggleAsset(a)}
                            className={`flex items-center justify-between rounded-lg px-3 py-2.5 border-2 cursor-pointer transition-all
                              ${
                                selected
                          ...
                          >
                            ...
                            <span className="text-xs bg-emerald-50 border border-emerald-200 text-emerald-700 px-2 py-0.5 rounded font-semibold">
                              AVAILABLE
                            </span>
                          </div>
                        );
            ...
            <div className="flex items-center gap-2 text-xs text-gray-400">
              ...
              {form.requested_assets?.length > 0 && (
                <span className="bg-blue-50 border border-blue-200 text-blue-700 px-2 py-0.5 rounded">
                  {form.requested_assets.length} asset
                  {form.requested_assets.length > 1 ? "s" : ""}
                </span>
              )}
            </div>
