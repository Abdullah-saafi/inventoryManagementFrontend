...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      const response = await fetchWithRefresh(`${import.meta.env.API}/users/login`, {
        method: "POST",
        ...
        <form className="login-form" onSubmit={handleSubmit}>
          ...
          <div className={`password-group ${message === "Invalid username or password" ? "incorrect" : message === "Password is required" ? "incorrect" : ""}`}>
            ...
            <div className="input-wrapper">
              <input className="login-form-input" type={showPassword ? "text" : "password"} name='password' id='password' placeholder='Enter Password' value={password} onChange={(e) => setPassword(e.target.value.trim())} />
